...
class CaptureVideoScreen extends GameCanvas {
	...
	public CaptureVideoScreen(MIDlet midlet, int type)  {
		super(false);
		System.out.println("[CapturevideoScreen:constructor] starting constructor..." );
		typescreen = type;
		System.out.println("[CapturevideoScreen:constructor] typescreen="+typescreen );

		display = Display.getDisplay(midlet);
		System.out.println("[CapturevideoScreen:constructor] display="+display);

		try {
			capturePlayer = Manager.createPlayer("capture://video");
			capturePlayer.realize();
			System.out.println("[CaptureVideoScreen:constructor] capture player="+capturePlayer);
		} catch (Exception e) {
			System.out.println("[CapturevideoScreen:constructor] It was not possible to create and/or initialize the player "+e.getMessage());
			e.printStackTrace();
		}

		videoControl = (VideoControl) capturePlayer.getControl("javax.microedition.media.control.VideoControl");
		System.out.println("[CapturevideoScreen:constructor] videoControl="+videoControl);
		try {
			...
			try {
				...
				System.out.println("[CapturevideoScreen:constructor] error while setting display "+e.getMessage());
				e.printStackTrace();
			}
	...
	public void startCapture() {
		try {
			if (!recording) {
				System.out.println("capturePlayer="+capturePlayer);
	...
	public void setVisibleVideo() {
		...
		try {
			...
			System.err.println("[CaptureVideoScreen:setVisibleVideo()]"+e.getMessage());
			e.printStackTrace();
		}
	}
}
